...
function formatLivBalance(balance: bigint): string {
  // LIV token uses 8 decimals (like ICP)
...
function WalletDropdown({
  ...
  const claimTokens = useClaimFounderLivTokens();
  ...
  const handleClaim = async () => {
    try {
      ...
      toast.success("🎉 Founder LIV tokens claimed successfully!");
    } catch (e: any) {
      toast.error(e?.message ?? "Failed to claim tokens");
    }
  };
    ...
    <div ref={dropdownRef} className="absolute right-0 top-full mt-2 z-50 w-80">
      ...
      >
        ...
        <div className="flex items-center gap-2 mb-4">
          ...
          <div>
            <p className="text-xs font-semibold text-foreground">My Wallet</p>
        ...
        <div className="flex flex-col gap-2">
          <a
            ...
            className="flex items-center gap-2 px-3 py-2.5 rounded-xl bg-muted/40 border border-border/30 hover:border-border/60 hover:bg-muted/60 transition-all text-sm text-foreground group"
            data-ocid="wallet.link"
          >
            <ExternalLink className="w-3.5 h-3.5 text-muted-foreground group-hover:text-foreground transition-colors" />
            <span>View ICP on OISY</span>
          </a>
          ...
          {isAdmin && (
            <button
              ...
              className="flex items-center justify-center gap-2 px-3 py-2.5 rounded-xl font-semibold text-sm transition-all disabled:opacity-60 disabled:cursor-not-allowed"
              style={{
                background:
                  "linear-gradient(135deg, oklch(0.65 0.18 148), oklch(0.55 0.16 165))",
                color: "white",
            ...
            >
              ...
              {claimTokens.isPending ? "Claiming..." : "Claim Founder Tokens"}
            </button>
          )}
        </div>
      </motion.div>
